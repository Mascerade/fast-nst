...
class BaseNSTConfig:
    def __init__(
        ...
        # Create data
        self.style_img = np.asarray(
            Image.open(self.style_img_path).resize(self.img_dim)
        ).transpose(2, 0, 1)[0:3]
    ...
    def compute_gram(self, matrix):
        ...
        return (1 / (channels * height * width)) * (
            torch.matmul(
                matrix.view(batches, channels, -1),
    ...
    def content_cost(self, input, target, precomputed):
        ...
        for layer, weight in enumerate(self.content_layers.values()):
            accumulated_loss = accumulated_loss + weight * torch.mean(
    ...
    def style_cost(self, input, target, precomputed):
        ...
        for layer, weight in enumerate(self.style_layers.values()):
            accumulated_loss = accumulated_loss + weight * torch.mean(
                torch.square(
                    self.compute_gram(input_layers[layer]) - target_layers[layer]
                )
            )
