...
class BaseNSTConfig():
    def __init__(self,
        ...
        # Create data
        self.style_img_np = np.asarray(Image.open(self.style_img_path).resize(self.img_dim)).transpose(2, 0, 1)[0:3]
        self.style_img_batch = self.style_img_np.reshape(1, 3, self.img_dim[0], self.img_dim[1])
        self.style_img_tensor = torch.from_numpy(np.copy(self.style_img_np)).float()
